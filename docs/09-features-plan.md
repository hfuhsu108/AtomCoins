...
| 批次 | 內容 | 規模 | 相依 | 狀態 |
|---|---|---|---|---|
| 1 | 搜尋篩選（＋6a 淨資產每日快照先行） | M | — | ✅ 完成（2026-07-21，驗收通過） |
| 2 | 範本（快速記帳） | M | — | ✅ 完成（2026-07-21，驗收通過） |
| 3 | 商家欄位＋別名＋商家排行 | M | — | ✅ 完成（2026-07-21，驗收通過） |
...
### 實作結果（2026-07-23，程式碼完成、待部署＋真機驗證）

**拍板決策**：情境 B 採「改爬蟲補 `newCount` → Firestore trigger 即時推」（非每日掃 invoices 退路）；VAPID 公鑰先留 `REPLACE_WITH_VAPID_PUBLIC_KEY` placeholder，待使用者產金鑰後替換 `functions/index.js` 與 `src/lib/push.js` 兩處。

**檔案**：
- 後端 `functions/`：`index.js`（`morningDigest` 09:00 C+D+E+F+G／`eveningNudge` 21:00 A／`onScraperStatus` trigger B／`sendTestPush` callable；web-push 發送、pushLog 去重、pushPrefs 逐情境檢查、410/404 清失效訂閱、region `asia-east1`、`taipeiToday()` 避 UTC 時區錯位）、`copy-shared.mjs`、`package.json`、`firebase.json`、`.firebaserc`（上個 session 已建 package.json／firebase.json／.firebaserc／shared 骨架）。
- 前端：`src/lib/push.js`（新）、`components/PwaProvider.jsx`（暴露 `swRegistration` state）、`lib/firebase.js`（`functions` export＝`asia-east1`）、`pages/SettingsPage.jsx`（「推播通知」子區塊）、`vite.config.js`＋`public/push-handler.js`（上個 session 已改）、共用純函式 `dueReminders`／`dueRecurringPostings` 在 `notifications.js`。
- 爬蟲：`atomcoins-scraper/firestore_upload.py` scraperStatus 補 `newCount`（=created）。
- schema：docs/01 §3.15／§3.16。
...
第二批（拍板：上下箭頭排序、刪分類改歸未分類、預設色盤、色套 icon＋報表）：
