...
### 6C 實作結果（2026-07-05 完成）

**已完成**：8 檔（5 新增 3 修改）。
- `src/db/repo.js`：`createInvoice`／`updateInvoice`／`deleteInvoice`／`recordInvoice`（歸帳，writeBatch 雙向 ref，範本＝`payCreditCardStatement`）／`unrecordInvoice`（取消歸帳，對稱刪交易防孤兒 ref）。
- `src/hooks/useScraperStatus.js`：獨立 `onSnapshot` 訂閱 `meta/scraperStatus`（meta 不在 DataProvider COLLECTIONS、文件無 id 欄，故不走 `useCollection`）。
- `src/components/invoice/{InvoicePanel,InvoiceRow,InvoiceEditSheet}.jsx`：主面板（同步條＋未歸帳/已處理切換）、列元件（動作按鈕：歸帳/略過/取消歸帳/復原，明細可展開）、手動新增/編輯 Sheet。
- `TransactionsPage`（接線 InvoicePanel）、`AddTransactionPage`（`?invoiceId=`）、`TransactionForm`（`initialInvoice` 預填＋明細對照＋歸帳 save 分支；歸帳時隱藏分期/週期）。

**歸帳語義**：生成 `type:'expense'` 走正常收支口徑；1 張發票→1 筆交易（transactionId 指主筆）；歸帳拆帳時新列自動帶入剩餘金額、拆帳明細顯示與發票原額差額，協助湊平。

**列操作用動作按鈕**（非 docs/04 字面的左滑；桌面 RWD 也適用、無手勢 lib 依賴，使用者定案）。

**表單截斷修復**（記帳＋歸帳共用 TransactionForm）：scroll body 加 `min-h-0`＋NumberPad `flex-none`（手機）；`AddTransactionPage` 桌面容器 `lg:h-auto`→`lg:h-[88vh]`（桌面 modal h-auto 會讓內部 flex 滾動失效）。
