...
**第八批後續調整（docs/09，2026-08-05，測試驗收通過，發布 v1.7.0）**：四項——① **設定頁分組**：`MENU` 11 項平鋪改成 `MENU_GROUPS` 五群（主檔設定／輸入輔助／偏好設定／資料同步＋主頁底部的關於與更新），**刻意維持單層**（分群只是視覺歸類，收成大類再進二層會讓每個設定都多點一次）；「外觀主題」新增 `theme` key 從主頁底部升為正式選單項，`TITLES` 改由 `flatMap` 推導，其餘 `section` 條件渲染零改動。② **`SwipeRow` 擴到標籤／券商／商家別名**（統一「點列＝編輯、左滑＝刪除」）；**帳戶與分類刻意不套**——列上有 `ReorderBtns`，而抽屜動作點完會 `close()`，排序進抽屜會變成「滑一次排一次」。刪除文案抽成 `tagDeleteMessage`／`brokerDeleteMessage` 具名匯出（兩個 Sheet 也從記帳表單的選擇器開啟，兩個入口都要能刪但文案只能一份）；`EmptyState` 要另包 `px-3.5`，否則 padding 移到列上後失去內距。③ **借貸期初餘額可事後調整**：拿掉 `{isNew &&}`，用 `txns.find(t => t.counterpartyId === cp.id && t.isOpening)` 找既有那筆預填——**「重複建立第二筆期初」的原顧慮由「找到就改同一筆」直接消解**；patch **只寫 `type`／`amount`／`tradeDate`／`postingDate`**，不寫 `repayments`／`isOpening`（patch 語義保留，免得陳舊快照覆寫他裝置剛登錄的還款）；兩個防呆＝金額不可低於已還款總額（`outstanding` 不可為負）、**有還款即鎖方向**（engine 依 `type` 決定 posting 正負，改方向會反轉既有還款的金流）；預填時序要把 `openingTx?.id` 併進 `lastKey`，否則交易訂閱晚一步到齊時欄位停在空白。順修 `TransactionForm` 借貸 payload 的 `accountId: initialTx?.isOpening ? null : accountId`（原本從借貸明細編輯存檔會讓 null 退回 `defaultAccountId`，金額不錯但破壞不變式）。④ **發票歸帳依歷史自動選帳戶**（原本不管哪家店都預選現金——`defaultAccountId` 只在 `seed.js` 寫成現金且全站無修改 UI）：`autoCategory.js` 新增 `suggestAccountFromHistory`，重用 `txMerchantName` 商家比對、投票對象換成 `tx.accountId`，**一筆一票**（與分類版每個拆帳列各一票的關鍵差異）；發票沒有卡號欄位故歷史交易是唯一推論來源；**刻意走前端即時算、不寫 `invoiceSuggestions`**（不需 LLM，即時算才吃得到剛記的帳）；`TransactionForm` 端**用 fallback 而非改 `stateFromInvoice` 初始值**（`useState` initializer 只跑一次，快取回填前算不出建議），手動選過即優先。離線驗算 12/12。**本輪 engine／date／notifications 零改動，不需 `firebase deploy --only functions`。**

**第九批後續調整（docs/09，2026-08-07，測試驗收通過，發布 v1.8.0）**：**發票 AI 分類的分歧降級＋拆帳建議**——使用者回報「付款方式用店家紀錄合理，但類別僅用店家不行，全聯買的東西會分到不同類別」。根因有三層：① `suggestFromHistory` 是**贏者全拿**（`tally.size > 0` 就回傳），全聯有任何一筆歷史就**永遠不會落到看得見品項的第二層 LLM** ② `confidence` 只看絕對票數，3/14 票也標 high，徽章反而讓錯誤建議更有說服力 ③ 建議 schema 只有單值 `categoryId`，但全聯的正解本來就是拆帳。四項改動：**分歧判定**（`dispersed`＝最高票佔比 <0.6 **或**與第二名差 <2 票；**只有一類時一律不分歧**，否則 `lead` 等於 `best.count` 會被誤判）＋`confidence` 改雙條件（`high` 需 `count≥3` **且** `share≥0.7`）＋`top` 前三名；**分歧商家降級送 LLM**，prompt 的品項改帶數量與金額（`衛生紙 ×2 =265`，只給名稱模型拆不出金額）、上限 8→20，並附該商家歷史分布當提示；**拆帳建議**（schema 擴 `splits`，寫入仍保留 `categoryId`＝`splits[0]` 使既有讀取端免遷移；`normalizeSplits` 硬門檻：至多 3 列、每列 ≥15% 且 ≥50 元、合計差 ≤1% 補進最大列否則退單列）；**前端**依 `splits` 開多列（`multiRow` 自動成立故 `SplitRows` 與「已湊平」列零改動）＋「這家店常用」快捷 chips。三個關鍵決定：**`normalizeSplits` 放 `autoCategory.js` 而非 `functions/index.js`**（後者有 firebase 相依、離線驗算不了，放共用檔既可測也避免判定散兩份）；**LLM 漏回時退回寫歷史建議**（不補這條會製造覆蓋率回歸——改前全聯至少有個不準的建議，改後可能什麼都沒有），但超過 `LLM_BATCH_CAP` 的 `pending` **刻意不寫退路**（寫了就進 `done`，下輪永遠不再送 LLM）；**分類建議的時序只能用 `useEffect` 補套**（不像帳戶建議可在渲染期當 fallback——分類是 state 結構），pristine 條件保證使用者動過就永不覆蓋。離線驗算 37/37。**本輪改到 `autoCategory.js`（在 `copy-shared.mjs` 清單），已一併 `firebase deploy --only functions`。**

**第十批後續調整（docs/09，2026-09-16，測試驗收通過，發布 v1.9.0）**：**信用卡不綁帳單的「繳款」＋新發票紅點**——使用者要「繳信用卡費可以自行輸入金額，也就是可以溢繳」「有新載具要有通知圓點」。查證發現繳費面板的金額**本來就能改、也沒有上限**，卡住的是入口（「繳費」鈕只在已出帳＋未繳＋金額>0 時出現，本期累計中／已繳過／金額 0 都按不到）。① **繳款與繳費分成兩個入口**：`payCreditCardStatement` 的 `period` 改為可省略，省略時只寫轉帳、不寫 `creditCardStatements` 快照——**直接放寬原本那顆鈕的顯示條件會寫出快照，本期還在累計時先預繳就會提早標已繳，`dueCardPayments` 跳過它，之後刷的消費永遠不再提醒繳費**；故期別卡「繳費」維持原顯示條件，額度摘要卡另加「繳款」（隨時可按、金額預帶已用額度）。`PaySheet` 改吃 `request`（`{ period }` 或 `{ period: null }`），金額與應繳不同時顯示多繳／少繳提示（警告＋允許，不擋存）。② **溢繳依裁示「只放寬入口，不抵扣」**（AskUserQuestion 選定；engine 與推播口徑零改動）；未來若要抵下期，**不能用卡片餘額推算**——分期 Model B 每期的銀行→卡轉帳在餘額上與繳款無從區分，只能用快照已存的 `paidAmount` 逐期推。③ **新發票紅點**：`lib/unseenInvoices.js` 純函式（`inbox`＋非 `manual`＋`createdAt` 晚於已讀）＋`useUnseenInvoices` 三處共用（底部導覽／側欄／分頁列），已讀時間存 `settings.invoiceSeenAt`（新欄位），`nextSeenAt` 取 max(現在, 未讀最晚 `createdAt`)——**只寫「現在」的話，裝置時鐘比爬蟲那台慢就永遠消不掉紅點**；爬蟲重抓既有 inbox 發票時保留原 `createdAt`，故同一張不會每天重新變新。離線驗算 15/15。**本輪 `engine.js`／`notifications.js`／`date.js` 零改動，不需 `firebase deploy --only functions`。**

**已部署**：2026-09-16 發布 **v1.9.0**（信用卡不綁帳單的「繳款」可隨時繳任意金額＋新發票紅點）。此前 2026-08-07 發布 **v1.8.0**（發票分類分歧降級送 LLM＋依品項金額建議拆帳＋常用分類快捷）。此前 2026-08-05 發布 **v1.7.0**（設定頁分組＋標籤/券商/別名左滑＋借貸期初可調＋發票歸帳自動選帳戶）。同日稍早發布 **v1.6.0**（週期規則事先設定＋範本設定頁管理＋圖示 1988＋24 色＋借貸期初餘額）。此前 2026-08-04 發布 **v1.5.0**（台股配息＋餘額調整＋日期欄位改善）。此前 2026-08-03 發布 **v1.4.0**（明細頁左滑動作列＋單擊預覽，取代長按預覽；Pixel 真機驗收通過）。此前 2026-08-01 發布 **v1.3.0**（UI 一致性修正 12 項＋長按預覽，真機驗收通過），線上 `https://hfuhsu108.github.io/AtomCoins/`（GitHub Actions 自動部署，push master 觸發）。此前 2026-07-28 發布 v1.2.0（六項使用體驗修補＋兩個信用卡帳單 bug 修復）、2026-07-26 發布 v1.1.4（專案標籤）與 v1.1.3（分類圖示自由選＋AI 發票分類）、2026-07-25 發布 v1.1.2（借貸功能補完）與 v1.1.1（批次 7 Web Push）、2026-07-21 發布 v1.1.0（docs/09 批次 1–6＋兩批後續調整）。
