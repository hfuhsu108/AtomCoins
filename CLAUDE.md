...
**待辦**：① **左滑動作列的真機驗收**——手勢類改動桌面滑鼠驗不準，待驗項目：垂直捲動不被吃掉、斜滑不誤開、iOS 左緣往右仍是系統返回、發票「歸帳」單擊一次就中、抽屜寬度（每顆 72px，卡片頁 3 顆＝216px 佔 375px 螢幕 58%，可能需調 `ACTION_W`）。驗收通過後才補：升版號、`git tag`、GitHub Release。② 第四批的線上驗收（含 AI 分類實際呼叫）。原「CSV 發票匯入」已於 2026-07-25 決定**取消**（爬蟲穩定運作，備援路徑無實際需求）。
