// Firestore CRUD 層（docs/07 M1）：與 repo.js 函式簽名一一對應，M2 切換時呼叫端零改動。
// 資料形狀（ISO 字串戳記、整數金額、nanoid id）與 Dexie 版完全一致，engine.js/calc.js 不感知差異。
import {
...
// 與 repo.js 相同：currency 預設 TWD；id 未給才產生；戳記最後覆寫不被 data 蓋掉
...
// 「銀行→卡」轉帳＋帳單繳款快照以 writeBatch 原子綁定，語義同 Dexie 版 db.transaction
...
// docId＝symbol（docs/07 §2-2），setDoc 覆寫語義同 Dexie put
